...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      for j in range(num_test):
        train_flat = x_train[i].view(-1)
        test_flat = x_test[j].view(-1)
        euc_dist = torch.sum((train_flat-test_flat)**2)
        dists[i,j] = euc_dist
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    for i in range(num_train):
      train_flat = x_train[i].view(1, -1)
      test_flat_vect = x_test.view(num_test, -1)
      euc_dist = torch.sum((train_flat-test_flat_vect)**2, dim=1)
      dists[i] = euc_dist
