...
def mutate_tensor(
    ...
    for i, j in indices:
      index = indices.index((i, j)) # Get the index of the tuple
      x[i, j] = values[index]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    for i in range(start, stop+1):
      if i % 10 == 0:
        arr.append(i)
    x = torch.tensor(arr).to(torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    #x[0:2, 2:6] = 2 # Why does it have to be mutated last
    x[2:4, 4:6:1] = 5
    x[2:4, 0:3:2] = 3
    x[2:4, 1:4:2] = 4
    x[0:2:1] = 0
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    for i in range(B):
      ...
      # Perform matrix multiplication for the i-th batch
      z[i] = torch.matmul(x_i, y_i)
    print("Printing z")
    print(z)
    ###########################################################################
    #                           END OF YOUR CODE                              #
